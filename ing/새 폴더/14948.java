...
//	BOJ - 14948
//	Algorithm - ?
//	2017 Sogang Programming Contest - Champion F번
...
public class Main {
	...
	public static int [][] permute(int [] source){
		...
		int [][] permute_ary = new int [(int)permute_num(n-1,m-1)][n+m-2];
	...
	public static long permute_num(int n,int m) {
	...
	private static class Path{
		...
		public void print() {
			...
			System.out.println("특수장비쓰는 곳: " + this.path[this.max_pivot] + "맥스피봇" + this.max_pivot);
	...
	private static class Map{
		...
		public Path path(int [] permute_ary) {
			...
			for(int i=1;i<n+m-2;i++) {
				if(permute_ary[i-1]!=permute_ary[i]) {
					path.curve[i] = true;
				}
				if(permute_ary[i-1]==1) {
					x_pivot++;
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<permute_num(n-1,m-1);i++) {
			...
			System.out.println(Arrays.toString(permute_ary[i]));
			map.path(permute_ary[i]).print();
			*/
			if(i==0) {
